...
export function App() {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    try {
      const saved = localStorage.getItem('kalaakar_products');
      if (saved) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('kalaakar_products', JSON.stringify(products));
    } catch (e) {
    ...
    <div className="min-h-screen flex flex-col bg-[#FAF9F6] text-[#1A1C1A] selection:bg-[#FFDCC4] selection:text-[#6F3800]">
      ...
      <main className="flex-grow pb-24 md:pb-12">
        ...
        {currentTab === 'settings' && (
          <SettingsView
            ...
            onToggleLanguage={() => setLanguage((l) => (l === 'en' ? 'hi' : 'en'))}
            artisan={artisan}
            onUpdateArtisan={(up) => setArtisan((prev) => ({ ...prev, ...up }))}
          />
        )}
      </main>
      ...
      {isArtisanSwitcherOpen && (
        <ArtisanSwitcherModal
          ...
          onSelectArtisan={(selected) => setArtisan(selected)}
